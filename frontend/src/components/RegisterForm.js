...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) return setMsg(data.error || "Error en el registro");
  ...
  return (
    <div
      className="flex items-center justify-center min-h-screen font-[Work Sans] px-4"
      style={{
        backgroundImage:
          step === 1
            ? `url(${im2})`
    ...
    >
      <div
        className="
          shadow-xl rounded-2xl flex bg-white overflow-hidden 
          w-full max-w-[1050px] max-h-[650px]
          md:h-[650px] 
      ...
      >
        ...
        >
          ...
          {step === 1 ? (
            <form onSubmit={handleSubmit} className="w-full max-w-sm mx-auto">
              <input className="input" type="text" name="nombre_empresa" placeholder="Nombre de Empresa" value={form.nombre_empresa} onChange={handleChange} />
              <input className="input" type="text" name="nit_empresa" placeholder="NIT" value={form.nit_empresa} maxLength="10"
                onChange={(e) => setForm({ ...form, nit_empresa: e.target.value.replace(/\D/g, "") })} />
              <input className="input" type="email" name="correo_contacto" placeholder="Correo Electrónico" value={form.correo_contacto} onChange={handleChange} />
              <input className="input" type="password" name="contrasena" placeholder="Contraseña" value={form.contrasena} onChange={handleChange} />
              <input className="input mb-6" type="password" name="confirmar_contrasena" placeholder="Confirmar Contraseña" value={form.confirmar_contrasena} onChange={handleChange} />
                ...
                onClick={async () => {
                  try {
                    ...
                    if (!res.ok) setMsgCodigo(data.error || "Error reenviando código");
                      ...
                      setMsgCodigo("Enviamos un nuevo código a tu correo");
                      setCodigo(Array(6).fill(""));
                    }
                  } catch (error) {
                    setMsg("Error de conexión con el servidor");
        ...
        <div className="hidden md:flex w-1/2 p-4 rounded-r-2xl">
          <img
